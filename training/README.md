...
- FC1-only Stage2 now runs a continuation loop instead of a single fixed block: check rollout data viability, train FC1 one block, evaluate true-state one-step skill/RMSE, then evaluate recursive RMSE growth. Failures are classified as `rollout_data_invalid`, `fc1_one_step_underfit`, or `fc1_recursive_instability`.
- Rollout data viability checks only target/input validity. Non-finite FC1 predictions are classified separately as `fc1_prediction_nonfinite`, so invalid simulation data is not confused with model-forward instability.
- FC1 gates use `R2` and persistence skill when target variance is usable; when `target_std < fc1_target_std_floor`, they switch to absolute RMSE thresholds instead of treating undefined `R2` as a hard failure.
- If the persistence baseline is nearly perfect (`baseline_rmse <= fc1_persistence_rmse_floor`), the one-step gate also uses absolute RMSE instead of requiring a finite persistence skill. Recursive rollout gating reads `fc1_rmse_growth_terminal`, with the actual available horizon logged separately.
